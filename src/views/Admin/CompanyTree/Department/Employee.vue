<script setup>
import List from "@/components/Admin/CompanyTree/Employee/List.vue";
import { useTreeApi } from "@/store/ApiServices/TreeApi";
import { useDepartmentStore } from "@/store/Admin/Company/Department";
import { onBeforeMount } from "vue";
import { useRoute } from "vue-router";
const route = useRoute();
const departmentStore = useDepartmentStore();
const treeApi = useTreeApi();

onBeforeMount(() => {
  treeApi.getEmployees(route.params.id, "department_id");
});
</script>
<template>
  <div class="d-flex justify-content-between align-items-center my-3">
    <h4 class="card-title">{{ $t("employees") }}</h4>
    <router-link
      type="button"
      class="btn btn-success text-light"
      :to="{
        name: 'employees-add',
        query: {
          musteri_id: departmentStore.department.company.musteri_id,
          company_id: departmentStore.department.company.id,
          company_id: departmentStore.department.id,
        },
      }"
    >
      <i class="fas fa-plus"></i> {{ $t("add") }}
    </router-link>
  </div>
  <List />
</template>
